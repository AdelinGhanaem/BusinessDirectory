...
//  }
  ...
  //TODO: try to add a list with 2500 entities.

  //TODO: crate a list editor for expenses .... !


  @Test
  public void searchWithSearchAPI() {
    Document.Builder documentBuilder = Document.newBuilder();

    documentBuilder.addField(Field.newBuilder().setName("field").setText("any"));

    Document document = documentBuilder.build();

    SearchService searchService = SearchServiceFactory.getSearchService();
    //getting and index from indexSpec. !
    IndexSpec indexSpec = IndexSpec.newBuilder().setName("document").build();
    Index index = searchService.getIndex(indexSpec);
    //the document is added to the index ...
    index.add(document);
    try {
      Results<ScoredDocument> results = index.search("field:any");
      for (ScoredDocument scoredDocuments : results) {
        System.out.println(scoredDocuments.getField("field"));
